...
# train_dataset.image_fns = train_dataset.image_fns[:10]
# val_dataset.image_fns = val_dataset.image_fns[:10]
...
for epoch in range(namespace.epochs):
    ...
    print('{}/{}    Train: {:.5f}    Val IOU: {:.5f}    Val AP: {:.5f}    lr: {:.9f}    T: {:.2f} s'.format(epoch+1, namespace.epochs, losses['train'][-1], losses['val-iou'][-1], losses['val-ap'][-1], optimizer.param_groups[-1]['lr'], time.time()-start))
